...
kthread_create(struct proc *p, kthread_func_t func, long arg1, void *arg2)
{
        ...
        dbg(DBG_PRINT, "GRADING1E: kthread_create\n");
        KASSERT(NULL != p);
        ...
        context_setup(&thr->kt_ctx,func,arg1,arg2,(void*)thr->kt_kstack,DEFAULT_STACK_SIZE,p->p_pagedir);
        return thr;
}
...
kthread_cancel(kthread_t *kthr, void *retval)
{
       /* NOT_YET_IMPLEMENTED("PROCS: kthread_cancel");*/
        dbg(DBG_PRINT, "GRADING1E: kthread_cancel\n");
        KASSERT(NULL != kthr);
        dbg(DBG_PRINT,"GRADING1A 3.b\n");
        if(kthr==curthr)
        {
             dbg(DBG_PRINT,"GRADING1E 16\n");
             kthread_exit(retval);/*modified*/
        }
        else if(kthr->kt_state==KT_SLEEP_CANCELLABLE)
        {
            dbg(DBG_PRINT,"GRADING1E 17\n");
...
kthread_exit(void *retval)
{
        ...
        dbg(DBG_PRINT, "GRADING1E: kthread_exit\n");
        
        curthr->kt_retval=retval;
        KASSERT(!curthr->kt_qlink.l_next && !curthr->kt_qlink.l_prev);
        dbg(DBG_PRINT,"GRADING1A 3.c-2\n");
        KASSERT(curthr->kt_proc == curproc);
        dbg(DBG_PRINT,"GRADING1A 3.c-3\n");
