...
hard_shutdown()
{
        ...
        int do_fabertest(kshell_t *kshell, int argc, char **argv)
        {
            KASSERT(kshell != NULL);
            dbg(DBG_PRINT, "do_fabertest() is invoked.\n");
            /*
             * Shouldn't call a test function directly.
             * It's best to invoke it in a separate kernel process.  
             */
        proc_t *newproc = proc_create("Faber test");
        kthread_t *newthr = kthread_create(newproc, faber_thread_test, 0, NULL);
        ...
        {
            KASSERT(kshell != NULL);
            dbg(DBG_PRINT, "do_sunghan test() is invoked.\n");
            /*
             * Shouldn't call a test function directly.
             * It's best to invoke it in a separate kernel process.  
             */
            proc_t *newproc = proc_create("sunghan test");
            kthread_t *newthr = kthread_create(newproc, sunghan_test, 0, NULL);
            sched_make_runnable(newthr);
            return 0;
        }
...
bootstrap(int arg1, void *arg2)
{
        ...
        dbg(DBG_PRINT, "GRADING1E: bootstrap\n");
        pt_template_init();
...
initproc_create(void)
{
        ...
        dbg(DBG_PRINT, "GRADING1E: initproc_create\n");
        proc_t *p_init=proc_create("Initial process");
        KASSERT(NULL !=p_init);
        dbg(DBG_PRINT,"GRADING1A 1.b-1\n");
        KASSERT(PID_INIT == p_init->p_pid);
        dbg(DBG_PRINT,"GRADING1A 1.b-2\n");
        kthread_t *thr_init=kthread_create(p_init,initproc_run,0,NULL);
        KASSERT(NULL !=thr_init); 
        dbg(DBG_PRINT,"GRADING1A 1.b-3\n");
        return thr_init;
}
