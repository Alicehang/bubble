...
proc_create(char *name)
{
    ...
    dbg(DBG_PROC, "Function: proc_create\n");
    memset(proc,0,sizeof(proc_t));
    pid_t pid=_proc_getid();
    proc->p_pid=pid;
    dbg(DBG_PROC, "Pid is:%d\n",pid);
    KASSERT(PID_IDLE != pid || list_empty(&_proc_list));
    if(proc->p_pid==PID_IDLE) curproc=proc;
    KASSERT(PID_INIT != pid || PID_IDLE == curproc->p_pid);
    if(pid==1) proc_initproc=proc;
    if(name!=NULL)
...
proc_cleanup(int status)
{
    ...
    dbg(DBG_PROC, "Function: proc_cleanup\n");
...
proc_kill(proc_t *p, int status)
{
    ...
    dbg(DBG_PROC, "Function: proc_kill,pid(%d)\n",p->p_pid);
    p->p_status=status;
    if(p==curproc)
       do_exit(status);
   else
...
proc_kill_all()
{
    ...
    dbg(DBG_PROC, "Function: proc_kill_all\n");
    proc_t* proc=NULL;
...
proc_thread_exited(void *retval)
{
    ...
    dbg(DBG_PROC, "Function: proc_thread_exited\n");
    if(retval!=NULL)
...
do_waitpid(pid_t pid, int options, int *status)
{
    ...
    dbg(DBG_PROC, "Function: do_waidpid\n");
    if(options!=0)
        return -1;
    if(pid!=-1&&pid<=0)
        return -1;
    ...
    else
    {
        ...
        while(1)
        {
            ...
            if(!find) return -ECHILD;
            else sched_sleep_on(&curproc->p_wait);
        }
    }
    return 0;
}
