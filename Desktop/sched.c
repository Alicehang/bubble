...
sched_wakeup_on(ktqueue_t *q)
{
      ...
      if(q->tq_size)
      {
          ktqueue_enqueue(kt_runq,q->tq_list.l_next);
          ktqueue_dequeue(q);
          return q->tq_list.l_next;
      }
        return NULL;
}
...
sched_make_runnable(kthread_t *thr)
{
	...
	uint8_t itr;
        itr=intr_getipl();
	intr_setipl(IPL_HIGH);
        ktqueue_enqueue(kt_runq->tq_list,thr);
	kt_runq->tq_size++;
        intr_setipl(itr);
